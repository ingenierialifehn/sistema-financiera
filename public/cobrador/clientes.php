...
<head>
    ...
    <title>Mis Clientes - Cobrador</title>
...
<body class="bg-gray-100">
    ...
    <header class="bg-indigo-600 text-white shadow-lg sticky top-0 z-50">
        <div class="px-4 py-3 flex items-center justify-between">
            ...
            <h1 class="text-lg font-bold">Mis Clientes</h1>
            <div class="w-16"></div>
        </div>
    </header>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.data.clientes.length > 0) {
                        ...
                        $('#clientesList').html('<div class="bg-white rounded-lg shadow p-4 text-center text-gray-500">No tienes clientes asignados</div>');
                    }
                },
                error: function() {
                    $('#clientesList').html('<div class="bg-white rounded-lg shadow p-4 text-center text-red-500">Error al cargar clientes</div>');
                }
            });
        }
                ...
                html += `
                    <div class="bg-white rounded-lg shadow p-4">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <h3 class="font-bold text-gray-800">${escapeHtml(cliente.nombre_completo || '')}</h3>
                                <p class="text-sm text-gray-500">${escapeHtml(cliente.codigo_cliente || '')}</p>
                                <p class="text-sm text-gray-500">${escapeHtml(cliente.tipo_documento || '')}: ${escapeHtml(cliente.numero_documento || '')}</p>
                                ${cliente.telefono ? `<p class="text-sm text-gray-500"><i class="fas fa-phone"></i> ${escapeHtml(cliente.telefono)}</p>` : ''}
                            </div>
                            <span class="px-2 py-1 text-xs rounded ${cliente.estado === 'activo' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}">
