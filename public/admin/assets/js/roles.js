/**
 * Roles y Permisos - JavaScript Mejorado
 * Incluye soporte para permisos detallados por acción
 */
        ...
        if (role.permisos.todos) {
            ...
            for (let key in role.permisos) {
                ...
                if (role.permisos[key] === true || (typeof role.permisos[key] === 'object' && role.permisos[key].view)) {
                    ...
                    // Count total actions
                    if (typeof role.permisos[key] === 'object') {
                        totalActions += Object.keys(role.permisos[key]).length;
                    }
                }
            }
            ...
            permisosHtml += `<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800">${activeModules} Módulos</span>`;
            permisosHtml += `<span class="px-2 ml-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">${totalActions} Acciones</span>`;
        }
...
function toggleAllActions() {
    ...
    const allCheckboxes = $('.permiso-check');
