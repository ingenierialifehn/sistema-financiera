...
try {
    ...
    $stmt = $db->prepare("
        ...
        VALUES (:usuario, :password, :nombre_completo, :email, 'cobrador', :estado)
    ");
    ...
    $cobradorId = $db->lastInsertId();
    
    // Registrar actividad
    Auth::logActivity($user['id'], 'create', 'cobrador', "Cobrador creado: {$validatedData['nombre_completo']}", null, $validatedData);
    
    // Obtener cobrador creado
    $getStmt = $db->prepare("
        SELECT id, usuario, nombre_completo, email, rol, estado, created_at
        FROM usuarios
        WHERE id = :id
    ");
    $getStmt->execute(['id' => $cobradorId]);
    $cobrador = $getStmt->fetch();
    
    Response::success($cobrador, 'Cobrador creado exitosamente');
    
} catch (Exception $e) {
    error_log("Error al crear cobrador: " . $e->getMessage());
    Response::serverError('Error al crear cobrador');
}
