<?php
/**
 * Script: Recalcular desglose de cuotas existentes
 * Fecha: 2026-01-08
 * Descripción: Aplica el desglose detallado a las cuotas pendientes de préstamos existentes
 */
...
try {
    ...
    echo "=== Iniciando recálculo de cuotas existentes ===\n\n";
    ...
    $stmt = $db->query("
        ...
        AND (c.capital_cuota IS NULL OR c.capital_cuota = 0)
    ");

    $prestamos = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $totalPrestamos = count($prestamos);

    echo "→ Encontrados $totalPrestamos préstamos con cuotas pendientes sin desglose\n\n";

    if ($totalPrestamos === 0) {
        echo "✓ No hay cuotas pendientes para recalcular.\n";
        exit(0);
    }
    ...
    foreach ($prestamos as $prestamo) {
        ...
        // Obtener cuotas pendientes de este préstamo
        $stmtCuotas = $db->prepare("
            SELECT id, monto_cuota, numero_cuota
            FROM cuotas
            WHERE prestamo_id = ?
        ...
        foreach ($cuotas as $cuota) {
            ...
            echo "   ✓ Cuota #{$cuota['numero_cuota']}: L " . number_format($montoCuota, 2) .
